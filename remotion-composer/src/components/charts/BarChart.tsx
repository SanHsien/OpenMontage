            ...
            return (
              <g key={`grid-${i}`}>
                ...
                <text
                  ...
                  opacity={gridOpacity}
                >
                  {formatNumber(line.value, decimals)}
                </text>
              </g>
            );
          })}
          ...
          return (
            <g key={datum.label} opacity={fadeOut}>
              ...
              {showValues && (
                ...
                >
                  {formatNumber(datum.value, decimals)}
                </text>
              )}
...
function formatNumber(n: number, decimals: number = 2): string {
  if (Math.abs(n) >= 1_000_000) return `${parseFloat((n / 1_000_000).toFixed(decimals))}M`;
  if (Math.abs(n) >= 1_000) return `${parseFloat((n / 1_000).toFixed(decimals))}K`;
