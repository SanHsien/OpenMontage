...
def test_readme_pair_cross_links_and_names_the_fork() -> None:
    ...
    assert "calesthio/OpenMontage" in zh
    assert "calesthio/OpenMontage" in en
...
def test_baseline_file_is_valid_and_complete() -> None:
    ...
    assert baseline["repo"] == "https://github.com/calesthio/OpenMontage.git"
    assert baseline["branch"] == "main"
    assert len(baseline["reviewed_through"]) == 40
    assert baseline["reviewed_through"] == "08e2151fa02de28a5d6a312b3d575692bf147ad7"
...
def test_baseline_matches_decisions_record() -> None:
    ...
    assert "08e2151" in upstream
    assert "calesthio/OpenMontage" in decisions