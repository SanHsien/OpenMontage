...
## 2026-09-13：上游 PR、Issue、分支全面評估與引進

2026-09-13 對 `calesthio/OpenMontage` 進行全面檢查與水位推進：

### 一、遠端分支審查
上游 27 條遠端分支已全數盤點：25 條已合併入 `upstream/main`；2 條未合併分支（`upstream/docs/readme-add-alexandria-remove-abyss` 與 `upstream/fix/backlot-ui-layout`）已評估，前者無功能價值略過，後者經比對已在 `main` 相關檔案中完全實現，無殘留修復。

### 二、本輪 PR 與 Issue 處理清單

| 項目 | 類型 | 評估結論 | 處置方式 |
|---|---|---|---|
| PR `#644` | PR | 補足缺失的音樂能力選擇器 `music_selector` | **引進合併**至本 fork |
| PR `#646` | PR | 意外刪除 `render_demo.py` docstring 開頭 | 評估拒絕（無效變更） |
| PR `#647` | PR | 修復 CostTracker 狀態機非法轉移 | **引進合併**至本 fork |
| PR `#648` | PR | 特定個人故事影片腳本（1.1 萬行），上游已關閉 | 評估拒絕（超出通用範疇） |
| PR `#651` | PR | 修復 Pexels / Pixabay 庫存提供者 Preflight 與空白變數判定 | **引進合併**至本 fork |
| PR `#652` | PR | 傳播 Archive.org 與 Wikimedia 傳輸錯誤至 corpus_builder | **引進合併**至本 fork |
| PR `#653` | PR | 修復 Pond5 契約（金鑰要求、Bearer 驗證、清除偽 fallback） | **引進合併**至本 fork |
| PR `#655` | PR | `.env` 白名單門控安全掃描（16 commits 激烈迭代中） | **暫緩引進（Defer）**，待上游穩定 |
| Issue `#654` | Issue | `BarChart` 與 `LineChart` 標籤數值截斷浮點數精度缺陷 | **修復完成**（保留兩位小數並去尾隨零） |

### 三、水位推進
- Commit 水位：`08e2151fa02de28a5d6a312b3d575692bf147ad7`
- PR 水位：`655`
- Issue 水位：`654`
